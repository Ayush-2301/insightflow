...
const WatchlistForm = ({
  ...
  const [loadingSuggestions, setLoadingSuggestions] = useState(
    !initialsuggestedKeywords // Initial loading state based on whether suggestedKeywords is undefined
  );
  const form = useForm<WatchlistForm>({
    resolver: zodResolver(watchlistFormSchema),
    defaultValues,
  ...
  useEffect(() => {
    const channel = supabase
      ...
      .on(
        "postgres_changes",
        {
          event: "INSERT", // Adjust based on your data model changes
          schema: "public",
          table: "master_keywords", // Adjust based on your data model
        },
        (payload) => {
          console.log(payload);
          ...
          setLoadingSuggestions(false); // Update loading state
        }
      )
      .subscribe();
